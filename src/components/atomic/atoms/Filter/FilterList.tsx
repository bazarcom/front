...
const FilterList = () => {
  ...
  const handleClick = () => {
    router.push(pathname + '?' + createQueryString('pview', 'list'), { scroll: false });
  };
  ...
  return (
    <div
      ...
      className={cn({
        ...
        'rounded border-[rgba(229,104,35,1)] bg-[rgba(242,173,151,0.13)]': isList,
      })}>
      <SvgList
        className={cn({
          'h-6 w-6 stroke-[rgba(62,78,80,1)]': true,
          'stroke-[rgba(229,104,35,1)]': isList,
        })}
      />
    </div>
  );
